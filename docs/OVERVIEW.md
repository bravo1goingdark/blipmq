...
- `blipmq_core` – broker core (topics, per-subscriber queues, QoS, TTL, retry).
- `blipmq_net` – TCP networking and binary framing (HELLO/AUTH, PUBLISH/SUBSCRIBE, POLL/ACK).
- `blipmq_wal` – write-ahead log for durable QoS1 messages.
- `blipmq_auth` – static API key validation.
- `blipmq_metrics` – HTTP `/metrics` endpoint.
- `blipmq_config` – configuration loader (TOML/YAML + env overrides).
- `blipmq_bench` – in-process benchmark harness using the real protocol.
- `blipmq_chaos` – chaos/failure testing utilities.
